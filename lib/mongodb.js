...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  if (s.ssh) {
    this.sshTunnel = tunnel({
      ...
      if (err) {
        throw new Error(g.f('Failed to create SSH tunnel: %s', err));
      }

      var tunneledOptions = Object.assign(s, {
        hostname: s.ssh.localHost,
        port: s.ssh.localPort,
      });

      initializeConnection(tunneledOptions, dataSource, callback);
    });
  } else {
...
MongoDB.prototype.parseUpdateData = function(model, data, options) {
  ...
  } else if (allowExtendedOperators !== false && modelClass && modelClass.settings && modelClass.settings.mongodb &&
...
MongoDB.prototype.idName = function(m) {
  return (m.definition && m.definition.idName()) || 'id';
}

MongoDB.prototype.idNames = function(m) {
  return (m.definition && m.definition.idNames()) || ['id'];
}
