...
function generateMongoDBURL(options) {
  ...
  options.hostname = getHostname(options);
  options.port = getPort(options);
  options.database = getDatabase(options);
...
function getHostname(options) {
  return options.hostname || options.host || '127.0.0.1';
}
...
/*!
 * Get database from options
 */
function getDatabase(options) {
  return options.database || options.db || 'test';
}

/*!
 * Initialize connection helper - used for both direct and SSH tunnel connections
 */
function initializeConnection(s, dataSource, callback) {
...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  if (s.ssh) {
    // SSH tunnel configuration for tunnel-ssh v5
    const privateKeyPath = s.ssh.privateKey || undefined;
    const localHost = s.ssh.localHost || '127.0.0.1';
    const localPort = s.ssh.localPort || 62102;

    // Create tunneled connection settings (will be used after tunnel is established)
    const tunneledSettings = Object.assign({}, s, {
      hostname: localHost,
      port: localPort,
    });

    // Set up the connector synchronously to satisfy loopback-datasource-juggler's postInit check
    // The actual connection will happen after the SSH tunnel is established
    tunneledSettings.url = tunneledSettings.url || generateMongoDBURL(tunneledSettings);
    dataSource.connector = new MongoDB(tunneledSettings, dataSource);
    dataSource.ObjectID = mongodb.ObjectId;

    // Tunnel server options
    const tunnelOptions = {
      autoClose: false, // We manage closing ourselves
    };

    // TCP server options (local binding)
    const serverOptions = {
      host: localHost,
      port: localPort,
    };
    ...
    const sshOptions = {
      host: s.ssh.host,
      port: parseInt(s.ssh.port, 10),
      username: s.ssh.username,
      privateKey: privateKeyPath ? fs.readFileSync(path.resolve(__dirname, privateKeyPath)) : undefined,
      passphrase: s.ssh.passphrase || undefined,
    };

    // Add password if provided (alternative to privateKey)
    if (s.ssh.password) {
      sshOptions.password = s.ssh.password;
    }

    // Add keepalive if configured
    if (s.ssh.keepAlive) {
      sshOptions.keepaliveInterval = 10000; // 10 seconds
    }
    ...
    try {
      createTunnel(tunnelOptions, serverOptions, sshOptions, forwardOptions)
        .then(
          ([server, conn]) => {
            debug('SSH tunnel established to %s:%d', s.ssh.host, s.ssh.port);

            // Store the tunnel server and connection for cleanup on disconnect
            dataSource.sshTunnel = {server, conn};

            // Now connect to MongoDB through the tunnel
            if (callback) {
              if (s.lazyConnect) {
                process.nextTick(function() {
                  callback();
          ...
          (err) => {
            // Use second argument to .then() to avoid catching errors from success handler
            const error = new Error(g.f('Failed to create SSH tunnel: %s', err.message || err));
            debug('SSH tunnel creation failed: %s', err.message || err);
            if (callback) {
              callback(error);
            } else {
              throw error;
      ...
      // Handle synchronous errors from createTunnel
      const error = new Error(g.f('Failed to create SSH tunnel: %s', syncErr.message || syncErr));
      debug('SSH tunnel sync error: %s', syncErr.message || syncErr);
      if (callback) {
        callback(error);
      } else {
        throw error;
    ...
    initializeConnection(s, dataSource, callback);
  }
};
...
MongoDB.prototype.parseUpdateData = function(modelName, data, options) {
  ...
  if (options.hasOwnProperty('allowExtendedOperators')) {
    ...
    modelClass && modelClass.settings && modelClass.settings.mongodb &&
...
MongoDB.prototype.updateOrCreate = function updateOrCreate(
  ...
  this.execute(
    ...
    buildOptions({
      ...
      includeResultMetadata: true, // mongodb v6+: preserve { value, lastErrorObject } shape
      sort: [['_id', 'asc']],
    }, options),
    function(err, result) {
      if (self.debug) {
...
MongoDB.prototype.findOrCreate = function findOrCreate(modelName, filter, data, options, callback) {
  ...
  const callbackFindOneAndUpdate = util.callbackify(() => this.collection(modelName).findOneAndUpdate(
    query,
    {$setOnInsert: data},
    {projection: projection, sort: sort, upsert: true, includeResultMetadata: true},
  ));
...
MongoDB.prototype.disconnect = function(cb) {
  ...
  */
  ...
  if (this.dataSource && this.dataSource.sshTunnel) {
    debug('Closing SSH tunnel');
    try {
      // tunnel-ssh v5 returns {server, conn} - close both
      if (this.dataSource.sshTunnel.server) {
        this.dataSource.sshTunnel.server.close();
      }
      if (this.dataSource.sshTunnel.conn) {
        this.dataSource.sshTunnel.conn.end();
      }
      this.dataSource.sshTunnel = null;
    } catch (err) {
      debug('Error closing SSH tunnel: %s', err.message || err);
    }
  }
