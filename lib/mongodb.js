...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  if (s.ssh) {
    ...
    const sshConfig = {
      ...
      dstHost: getHostname(s),
      dstPort: getPort(s),
      localHost: s.ssh.localHost || '127.0.0.1',
      localPort: s.ssh.localPort || 62102,
      keepAlive: s.ssh.keepAlive || false,
    };
    ...
    tunnel(sshConfig, function(err, server) {
      ...
      // Store the tunnel server for cleanup on disconnect
      dataSource.sshTunnel = server;

      // Create tunneled connection settings
      const tunneledSettings = Object.assign({}, s, {
        hostname: s.ssh.localHost || '127.0.0.1',
        port: s.ssh.localPort || 62102,
      });
...
MongoDB.prototype.disconnect = function(cb) {
  ...
  */
  ...
  if (this.dataSource && this.dataSource.sshTunnel) {
    ...
    try {
      this.dataSource.sshTunnel.close();
